...
class UncertaintyViaNetwork(UncertaintyWrapper):
    ...
    def forward(self, *args, patches=False,**kwargs):
        if not patches:
            features = self.model_forward_head(self.model_forward_features(*args, **kwargs), pre_logits=True)
            out = self.model.get_classifier()(features)
            ...
            features = self.model_forward_features(*args, **kwargs)
            tokens = features[:,1:,:]
            outs = torch.zeros(tokens.shape[0],tokens.shape[1],1).to(tokens.device)
            for token_idx in range(tokens.shape[1]):
                token = tokens[:,token_idx,:]
                token = self.model.fc_norm(token)
                token = self.model.head_drop(token)
                unc = self.unc_module(token if not self.stopgrad else token.detach()).squeeze()
                unc = unc * self.unc_scaler
                outs[:,token_idx,0] = unc
            features = self.model_forward_head(features, pre_logits=True)
            out = self.model.get_classifier()(features)
            unc = self.unc_module(features if not self.stopgrad else features.detach()).squeeze()
            unc = unc * self.unc_scaler
            return ((out, unc, features), outs)




class UncertaintyTransformer(nn.Module):
    def __init__(self, in_channels=2048, width=512, depth=3, pool='mean',init_prednet_zero=False) -> None:
        super().__init__()
        
        self.unc_module = Block(
            dim=in_channels,                 # Embedding dimension
            num_heads=2,     # Number of attention heads
            mlp_ratio=4.,     # Ratio of hidden dim to embedding dim in MLP
            qkv_bias=False,       # Bias for query, key, value projections
        )
        ...
        if init_prednet_zero:
            ...
            self.head.apply(self.init_weights_zero)
        
    def forward(self, input):
        x = self.unc_module(input)
        if self.pool == 'mean':
            x = x.mean(dim=1)
        else:
            x = x[:,0,:]
        return self.EPS + self.head(x)
...
class UncertaintyViaTransformer(UncertaintyWrapper):
    def __init__(self, model, stopgrad=False, *args, **kwargs):
        super().__init__(model)
        self.unc_module = UncertaintyTransformer(
            in_channels=model.num_features, *args, **kwargs
        )
        self.stopgrad=stopgrad
    ...
    def forward(self, *args, patches=False,**kwargs):
        if not patches:
            features = self.model_forward_features(args[0], **kwargs)
            if len(args) == 2 and self._is_processed_embedding(args[1]):
                tokens = args[1]
            else:
                tokens = features[:,1:,:]
            features = self.model_forward_head(features, pre_logits=True)
            out = self.model.get_classifier()(features)
            unc = self.unc_module(tokens if not self.stopgrad else tokens.detach()).squeeze()
            unc = unc * self.unc_scaler
            return out, unc, features
        else:
            features = self.model_forward_features(*args, **kwargs)
            ...
            for token_idx in range(tokens.shape[1]):
                ...
                token = self.model.fc_norm(token)
                token = self.model.head_drop(token)
                unc = self.unc_module(token.unsqueeze(1) if not self.stopgrad else token.detach()).squeeze()
                unc = unc * self.unc_scaler
                outs[:,token_idx,0] = unc
            unc = self.unc_module(tokens if not self.stopgrad else tokens.detach()).squeeze()
            unc = unc * self.unc_scaler
