...
class GenDEBRIS(Dataset): # Extend PyTorch's Dataset class
    def __init__(self, configs, dataset, mode): #mode = 'train', transform=None, standardization=None, path = dataset_path, agg_to_water= True):
        self.configs = configs
        self.name = dataset
        if "augment" in self.configs and self.configs["augment"]:
            self.transform = utilities.augmentations.get_augmentations(configs)
        else:
            self.transform = None
            
        self.root_path = os.path.join(configs["dataset_root_path"], "MARIDA")
        agg_to_water= True
        path = self.root_path
        ...
        self.mode = mode
        if "normalization" in self.configs and self.configs["normalization"] is not None and self.configs['normalization'] == "standard":
...
if __name__ == '__main__':
    dataset = GenDEBRIS(mode = 'train', transform=None, standardization=None, path = "/mnt/nvme1/nbountos/MARIDA", agg_to_water= True)
    print(len(dataset))
