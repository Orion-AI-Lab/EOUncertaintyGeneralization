...
class WoodyDataset(torch.utils.data.Dataset):
    def __init__(self, configs, dataset, mode="train"):
        ...
        self.root_path = os.path.join(configs["dataset_root_path"], "Woody")
        self.mode = mode
        self.samples = []
        self.full_samples = []
        self.test_captions = ["pinus_f2_ortho", "ulex_f4_ortho", "acacia_f1_ortho"]
        if "augment" in configs and self.configs["augment"]:
    ...
    def __getitem__(self, index):
        ...
        if not self.configs["webdataset"]:
            if "augment" in self.configs and self.configs["augment"] and self.mode == "train":
                image = einops.rearrange(image, "c h w -> h w c")
