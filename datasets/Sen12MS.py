...
class Sen12MSDataset(torch.utils.data.Dataset):
    def __init__(self, configs, dataset, mode="train"):
        ...
        self.root_path = os.path.join(configs["dataset_root_path"], "sen12ms")
        self.mode = mode
        ...
        self.name = dataset
        if configs["task"]!="wds_write_parallel":
            if self.configs["normalization"] == "standard":
                self.normalization = transforms.Normalize(ean=self.configs[self.name + "_stats"]["mean"], std=self.configs[self.name + "_stats"]["std"])
    ...
    def plot(self, index=0):
        ...
        labels[labels >= self.threshold] = 1
        labels[labels < self.threshold] = 0
    ...
    def __getitem__(self, index):
        ...
        with rasterio.open(s2_path) as srcs2:
            s2_patch = srcs2.read()
        labels = np.asarray(sample["labels"].copy())
